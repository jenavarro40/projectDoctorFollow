...
class FirestoreHelper {
    ...
    fun foodRequest(context: Context, request: FoodRequest ) {
            ...
            .addOnSuccessListener { documents ->
                if (documents.isEmpty){
                    db.collection("foodRequest")
                        .add(request)
                        .addOnSuccessListener { documentReference ->
                            Log.d("FirestoreHelper", "Requested food: ${request.name}")
                            Toast.makeText(context, "Requested food: ${documentReference.id}", Toast.LENGTH_SHORT).show()
                        }
                        .addOnFailureListener { e ->
                            Log.w("FirestoreHelper", "Error Requestedfood", e)
                        }

                }
                else {
                    val docId = documents.documents[0].id
                    val updateMap = mapOf("foodRequest" to request.nutritionist)

                    db.collection("foodRequest").document(docId)
                        .update(updateMap)
                        .addOnSuccessListener {
                            Log.d("FirestoreHelper", "Requested food updated for: ${request.email}")
                            Toast.makeText(context, "Requested food updated", Toast.LENGTH_SHORT).show()
                        }
                        .addOnFailureListener { e ->
                            Log.w("FirestoreHelper", "Error updating food request", e)
                        }
                }
            }
            .addOnFailureListener { exception ->
    ...
    fun getfoodRequest(context: Context,userMail:String, result: (FoodRequest) -> Unit) {
        db.collection("foodRequest")
            .whereEqualTo("email", userMail)
            .limit(1)
            .get()
            .addOnSuccessListener { documents ->
                if (!documents.isEmpty) {
                    val foods = documents.first().toObject(FoodRequest::class.java)
                    result(foods)

                } else {
                    Toast.makeText(context, "No data for nutritionist", Toast.LENGTH_SHORT).show()
                    result(FoodRequest())
                }
            }
            .addOnFailureListener { exception ->

                Log.w("FirestoreHelper", "Error in read food", exception)
            }
    }
            ...
            db.collection("users")
                ...
                .addOnSuccessListener { documents ->
                    if (!documents.isEmpty) {
                        ...
                        Toast.makeText(
                            context,
                            "There was aproblem please Check",
                            Toast.LENGTH_SHORT
                        ).show()
                        result(users())
                    }
        ...
        fun insertKPI(context: Context, insert: InsertKpi) {
            db.collection("InsertKPI")
                ...
                .addOnSuccessListener { documentReference ->
                    ...
                    Toast.makeText(
                        context,
                        "Insert Data of: ${insert.name} at ${insert.date}",
                        Toast.LENGTH_SHORT
                    ).show()
                }
                .addOnFailureListener { e ->
            ...
            db.collection("InsertKPI")
                ...
                .addOnSuccessListener { documents ->
                    if (!documents.isEmpty) {
                        ...
                        Toast.makeText(
                            context,
                            "There is not data for ${userMail}",
                            Toast.LENGTH_SHORT
                        ).show()
                    ...
                    for (fileRef in userFiles) {
                        ...
                        fileRef.getFile(localFile)
                            .addOnSuccessListener {
                                Toast.makeText(
                                    context,
                                    "Files from ${fileRef.name} download",
                                    Toast.LENGTH_SHORT
                                ).show()
                            }
                            .addOnFailureListener { e ->
                                Log.e(
                                    "FirestoreHelper",
                                    "Error download ${fileRef.name}: ${e.message}"
                                )
                            }
                    }
                }
